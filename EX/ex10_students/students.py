...
class Student:
    ...
    def __init__(self, name, list_of_courses, average_grade):
        """Student constructor."""
        self.name = name
...
def filter_by_course(student_list: list, course: str) -> list:
    ...
    """
    list_of_needed_students = []
    for student in student_list:
        if course in student.courses:
            list_of_needed_students.append(student)
    return list_of_needed_students
...
def succeeding_students(student_list: list) -> list:
    ...
    """
    list_of_successful_students = []
    for student in student_list:
        if is_failing(student) is False:
            list_of_successful_students.append(student)
    return list_of_successful_students
...
def failing_students(student_list: list) -> list:
    ...
    """
    list_of_not_successful_students = []
    for student in student_list:
        if is_failing(student) is True:
            list_of_not_successful_students.append(student)
    return list_of_not_successful_students
...
def sort_by_best_grade(student_list: list) -> list:
    ...
    """
    for index, student0 in enumerate(student_list):
        if is_failing(student0) is True:
            student_list.pop(index)
    return sorted(student_list, key=lambda student: (-student.average_grade, student.name))
