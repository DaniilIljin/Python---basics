...
def make_pyramid(base: int, char: str) -> list:
    ...
    if base >= 0:
        ...
        pyramid = [[' ' for i in range(int((base - element) / 2))] + [char for i in range(element)] + [' ' for i in range(int((base - element) / 2))] for element in amounts_of_char_in_list]
        return pyramid
    else:
        return []
